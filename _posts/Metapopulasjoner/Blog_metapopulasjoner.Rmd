...
En bestand er sjeldent alene. Man finner ofte flere små bestander av samme art spredt utover landskaper. Noen bestander er store, andre små. Noen som ligger tett sammen, andre som ligger mer isolert. Felles kalles disse bestandene (populasjonene) for en *metapopulasjon* (@levins1969some, @levin1970community), eller 'populasjon av populasjoner'. Hvor landskapet mellom bestandene blir omtalt som 'matrisen'. 
 

Hvis en ser $N$ bestander som kan finnes på $T$ forskjellige steder i landskapet vil $x$ bestander bli utryddet hvert år (utryddelsesrate), men $m'$ nye bestander etableres hvert år (immigrasjonsrate). Endring i antall bestander hvert år ($t$) kan da beskrives som
...
```{r FIG1, echo = T, fig.cap="Hvordan antall inndvandringer og utvandringer er avhengig av avstand til fastland og arealet."}
...
ParameterSpace = expand.grid(nIterations = 1:nSimulations,
                             ...
                             RangeKonstantC = seq(.25,.75,length.out =  1),
                             RangeKonstantE = seq(325,100,length.out = 1),
...
for(i in 1:nrow(ParameterSpace)){
  ...
  TidsArray = array(rep(0, tmax*nØyer), 
                                      dim = c(tmax, nØyer, max(ParameterSpace$nIterations)))
  ...
  while(t<tmax){
    ...
    TidsserieKoloniseringsrater[t,,iteration]<-KoloniseringsRateØy_i
    ...
    Tilstede<-Kolonisert
    TidsserieTilstede[t,]<-Tilstede
    TidsArray[t,,iteration]<-Tilstede
...
Men hvilke øyer er viktige? Og hva kan vi gjøre for å styrke metapopulasjonen? Det er spørsmål vi får gyve løs på senere. 
