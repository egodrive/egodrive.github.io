---
title: "Nisjeteori: Location, location, location"
description: > 
...
# Nisje
*Fundamental nisje* er den kombinasjoen av miljøvariabler som tillater at en art å overleve. Veldig ofte vil temperatur være en viktig variabel. Det må som regel være passe varmt. Ikke for kaldt, men samtidig ikke for varmt. Videre så må det være tilgang på den og den maten, sommeren må være passe lang, og vinteren må innebære hvertfall en halv meter snø som ligger mer enn 3 måneder og bakken kan ikke ha for mye kalk i seg. 
Slik at i møte med flere miljøvariabler vil en art bare forekomme i et begrenset omfang (figur \@ref(fig:FIG1)). 

```{r FIG1, fig.height = 5, fig.width = 15, fig.align = "center", echo = F, fig.cap="En art finnes ikke overalt, bare hvor den kan overleve og reprodusere."}
...
```
I møte med flere arter vil det også oppstå interaksjoner som kan utvide eller begrense nisjen til en art (figur \@ref(fig:FIG2)). . Gjennom konkurranse kan en art fortrenge en annen. En snakker da om den *realiserte nisjen* til en art. 


```{r FIG2, fig.height = 5, fig.width = 15, fig.align = "center", echo = F, fig.cap="En art finnes ikke overalt, bare hvor den kan overleve og reprodusere."}
...
```
*Biotop* er når en utvider habitatkonseptet til flere arter (@Whittaker1973). Hvor art 1 og 2 i figuren utgjør sammen med den den delen av miljøvariabelen de befinner seg på et biotop. Dette er et system som også går igjen i naturtype-systemet i Norge: [Natur i Norge (NiN)](https://www.artsdatabanken.no/NiN). Hvor såkalte *lokale komplekse miljøvariabler* (forferdelig lite pedagogisk navn) tilsvarer en miljøvariabel som for eksempel kalkinnhold. Her vil f.eks. fastmarksskogsmark bli kalt for et *hovedtypegruppe* (også kalt for et økosystem i @halvorsen2020), men en skog med gitt kalkinnhold og uttørkingsfare kan være [kalklågurt-skog](https://www.artsdatabanken.no/Pages/171923/Fastmarksskogsmark). Kalklågurt-skog vil være en *naturtype*, men også en biotop for den karakteriseres av de artene man finner innenfor gitte miljøparametre. Dette er basert på det [generaliserte artslistedatasett](https://artsdatabanken.no/Pages/200275).  
...
```{r FIG3, fig.height = 5, fig.width = 15, fig.align = "center", echo = F, fig.cap="Fordel (blå), ulempe (rød) og nytte (grønn) knyttet til bruken av et miljø."}
library(boot)
library(boot)
...
curve(inv.logit(2+.35*x), from = -2, to = 2, ylim = c(0,1), col = "blue", xlab = "Miljøvariabel 1",
      ylab = "Fordel/Ulempe", lwd = 2, cex.lab = 1.7, cex.axis = 1.3)
...
curve(inv.logit(.25+3.35*x), from = -2, to = 2, col = "blue", xlab = "Miljøvariabel 1",
      ylab = "Fordel/Ulempe", lwd = 2, cex.lab = 1.7, cex.axis = 1.3)
abline(v = 0, lty = "dashed")
...
Dette har også likheter med øybiogeografi (@macarthur2016theory).
